...
Случай A — ты уверен и можешь сгенерировать SQL:
{
  ...
  • "sql" — ОДИН валидный SELECT-запрос на диалекте Trino SQL, без точки с запятой в конце. Важно: используй полные имена таблиц в формате `каталог.схема.таблица` (например, `postgresql.public.employees`). В качестве имени каталога используй СТРОГО значение из поля "catalog" (например, `cat_...`), а НЕ "catalog_alias".
  ...
  • "explanation" — 1–3 предложения на языке пользователя. ВАЖНО: Различай понятия "Имя" (First name) и "ФИО" (Full name). Если колонка называется `fio` и содержит полные ФИО, пиши в объяснении "ФИО" или "полное имя", а не просто "имя". Если пользователь просит отсортировать или найти именно по ИМЕНИ (First name), то ты должен извлекать второе слово из колонки ФИО с помощью `split_part(fio, ' ', 2)`, а не делать `ORDER BY fio` (так как `ORDER BY fio` отсортирует по фамилии).
...
Диалект: Trino SQL (Presto). Используй функции Trino и полные пути к таблицам: каталог.схема.таблица (в качестве каталога бери строго поле "catalog", а не "catalog_alias").
